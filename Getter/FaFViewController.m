        ...
        if (reqestdata_count == 1)
        {
            if(self.tableView.contentOffset.y >= (500*number_count - self.tableView.bounds.size.height))
            {
                ...
                if(followinglistFaF == NULL){
                    ...
                    fafURL = followerurl;
                } else if(followerlistFaF == NULL){
                    NSString *str_cid_followinglist = [NSString stringWithFormat:@"https://api.twitter.com/1.1/friends/list.json?screen_name=%@&cursor=%@",scname_followerlist,next_cursor_data];
                    NSURL *followingurl = [NSURL URLWithString:[str_cid_followinglist stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding]];
                    fafURL = followingurl;
                }
                //NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:followerurl];
                request = [NSMutableURLRequest requestWithURL:fafURL];
